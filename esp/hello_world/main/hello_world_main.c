...
void servo_task(void *pvParameter)
{
    ...
    while(1)
    {
        ...
        int degree = (adc_value / 4095.0) * SERVO_MAX_DEGREE; //Cálculo del ángulo correspondiente

        int duty_us = (degree / (float) SERVO_MAX_DEGREE) * (SERVO_MAX_PULSEWIDTH - SERVO_MIN_PULSEWIDTH) + SERVO_MIN_PULSEWIDTH; //Cálculo del ciclo de trabajo para el servo
